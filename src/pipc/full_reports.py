...
def render_full_index(df: pd.DataFrame) -> str:
    ...
    type_links = [
        ("enforcement.html", "법규 위반·제재", "제재 조합, 금액구간, 조문쌍, 사건유형·factor"),
        ("privacy_impact.html", "침해요인 평가", "신청기관, 법령명, 권고율, 반복 쟁점"),
        ("public_system.html", "공공시스템·실태점검", "실태점검 흐름, 개선권고, 기관·제목군"),
        ("data_provision.html", "개인정보 제공 요청", "허용/불허, 영상정보, 목적 외 제공 쟁점"),
        ("interpretation_other.html", "민원·해석·기타", "해석성 안건, 정책성 안건, 조문 중심 쟁점"),
    ]
    body += "<section><h2>결정문 유형별 보고서</h2><div class='link-grid'>"
    for href, title, desc in type_links:
        body += f"<a class='report-link' href='{href}'><strong>{title}</strong><span>{desc}</span></a>"
    body += "</div></section>"
    body += section(
        "전체 관찰",
        insight_list(
            [
                f"전체 {len(df):,}건 중 최대 유형은 침해요인 평가이며, 법령 제·개정 단계의 개인정보 처리 설계 쟁점이 큰 비중을 차지한다.",
                "2022년 이후 결정문 수가 크게 늘어 단순 전체 비율보다 시기별·유형별 비교가 실무 판단에 더 유효하다.",
                "데이터 품질과 전체 구조 분석은 결정문 유형이 아니라 검수·운영 관점이므로 별도 관리 메뉴로 분리했다.",
            ]
        ),
    )
    body += note("클러스터링은 별도 메뉴가 아니라 각 결정문 유형 내부의 토픽 지도로 배치했다. OpenRouter 임베딩, UMAP, HDBSCAN, LLM 라벨링을 사용한다.")
...
def overview_insights(df: pd.DataFrame) -> list[str]:
    ...
    return [
        ...
        "실무 활용은 전체 검색보다 장르별 검색·쟁점 비교 화면을 제공하는 방향이 적합하다.",
    ]
